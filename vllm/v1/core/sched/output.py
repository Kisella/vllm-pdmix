...
class HiddenChannelType:
    ...
    Two generations of channel values co-exist:

    * The six fixed directional channels (``PREFILL_UP``/``PREFILL_DOWN``,
      ``PREFILL_DRAFT_UP``/``PREFILL_DRAFT_DOWN``, ``DECODE_UP``/
      ``DECODE_DOWN``) are the physical wires of the edge-cloud comm
      layer: each maps 1:1 to a dedicated HCCL communicator + NPU
      stream, carrying exactly one task type in one direction.
    * The legacy ``PREFILL_i`` / ``DECODE_i`` pools (generated by
      ``init``) remain for the current scheduler's in-flight
      bookkeeping labels only; the comm layer no longer resolves them
      to physical groups.  ``init`` is idempotent and can be called
      again during engine bootstrap.
    """
    ...
    def init(
        ...
        # The six fixed directional channels (type x direction).  Each is
        # a dedicated physical wire (communicator + stream) of the
        # edge-cloud comm layer; values match CommChannelType exactly so
        # the logical->physical mapping is the identity.
        for name in cls.SIX_DIRECTIONAL_CHANNELS:
            setattr(cls, name, cls(name.lower()))

    # Names of the six fixed directional channels (see init()).
    SIX_DIRECTIONAL_CHANNELS: tuple[str, ...] = (
        "PREFILL_UP",
        "PREFILL_DOWN",
        "PREFILL_DRAFT_UP",
        "PREFILL_DRAFT_DOWN",
